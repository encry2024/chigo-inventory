...
@section ('title', trans('labels.backend.inventory.customer.management') . ' | ' . trans('labels.backend.inventory.customer.edit'))

@section('page-header')
<h1>
   {{ trans('labels.backend.inventory.customer.management') }}
   <small>{{ trans('labels.backend.inventory.customer.edit') }}</small>
</h1>
@endsection

@section('content')
{{ Form::model($customer, ['route' => ['admin.inventory.customer.update', $customer], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) }}

<div class="box box-success">
   <div class="box-header with-border">
      <h3 class="box-title">{{ trans('labels.backend.inventory.customer.edit') }}</h3>
