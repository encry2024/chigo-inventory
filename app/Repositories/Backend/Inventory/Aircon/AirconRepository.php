    ...
    public function import(Model $aircon, array $input)
    {
        ...
        DB::transaction(function () use ($aircon, $data) {
            ...
            foreach ($airconFile->toArray() as $key => $value) {
                $insert[] = [
                    'name'             => isset($value['name']) ? '0.0' : $value['name'],
                    'customer_id'      => 0,
                    'manufacturer'     => isset($value['manufacturer']) ? '0.0' : $value['manufacturer'],
                    'price'            => isset($value['selling_price']) ? '0.0' : $value['selling_price'],
                    'selling_price'    => $value['selling_price'],
                    'horsepower'       => isset($value['horsepower']) ? '0.0' : $value['horsepower'],
                    'voltage'          => isset($value['voltage']) ? '0.0' : $value['voltage'],
                    'size'             => isset($value['size']) ? '0.0' : $value['size'],
                    'brand_name'       => isset($value['selling_price']) ? '-' : $value['selling_price'],
                    'feature'          => isset($value['selling_price']) ? '-' : $value['selling_price'],
                    'status'           => 2,
